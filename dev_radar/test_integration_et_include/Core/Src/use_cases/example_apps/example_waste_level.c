...
void waste_level_process(waste_level_handle_t *handle, const waste_level_app_config_t *app_config,
                         const acc_processing_metadata_t *metadata, const acc_int16_complex_t *frame,
                         waste_level_result_t *waste_level_result)
{
	if (handle != NULL && app_config != NULL && metadata != NULL && frame != NULL && waste_level_result != NULL)
	{
		...
		float new_distance = !point_of_waste_found ? NAN : point_in_sweep_to_distance_m(app_config->sensor_config,
		                                                                                metadata,
		                                                                                point_of_waste);
		...
		uint16_t num_normal = copy_non_nan_floats(handle->state.distance_history_points,
		                                          handle->scratch,
		                                          handle->state.distance_history_len);
		if (num_normal > 0U)
		{
...
static bool validate_app_config(const waste_level_app_config_t *app_config)
{
	...
	{
		printf("Bin end needs to be greater than bin start\n");
		status = false;
	}

	if (status && (processing_config->threshold < 0.0f || processing_config->threshold > 4.0f))
	{
		printf("Threshold shoud be in the range [0.0, 4.0], was %" PRIfloat "\n",
	...
	{
		printf("Distance sequence length shoud be in the range [1, 10], was %" PRIu16 "\n",
	...
	{
		printf("Median filter length shoud be in the range [1, 10], was %" PRIu16 "\n",
	...
	{
		printf("Subsweeps with higher indexes should measure further away than those with lower indexes\n");
		status = false;
	}
	...
	if (status)
	{
		...
		{
			printf("Disallowed range overlap between susweeps %" PRIu8 " and %" PRIu8 "\n",
			       subsweep_idx,
			       subsweep_idx + 1U);
			status = false;
		}
	...
	{
		printf("Bin start is outside measuring interval\n");
		status = false;
	}

	if (status && is_outside_measuring_interval(sensor_config, processing_config->bin_end_m))
	{
		printf("Bin end is outside measuring interval\n");
		status = false;
	}

	if (status && acc_config_sweeps_per_frame_get(sensor_config) <= 3)
	{
		printf("Sweeps per frame needs to be greater than 3\n");
		status = false;
	}
...
static void set_level_numbers(float filtered_distance, const waste_level_processing_config_t *processing_config, waste_level_result_t *result)
{
	...
	float fill_level_percent_f = acc_algorithm_clip_f32(100.0f * fill_level_m / (bin_end_m - bin_start_m),
	                                                    0.0f,
	                                                    100.0f);
